...
$query = "SELECT * FROM jobs JOIN employers ON jobs.Employer_ID_FK = employers.Employer_ID JOIN employers_mobile_number ON employers_mobile_number.Employer_ID_FK = employers.Employer_ID where Employee_Type_ID_FK = '$user_type'";
    ...
    <div class="container-fluid">  
        ...
        <div class="col-lg-12">
          <div id="form_ta" class="card">
            <div class="card-body">
              <h5 class="card-title">Job Bit Details</h5>
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <tr>
                    ...
                    <?php 
                        $jobid = $row['Job_ID'];
                        $empID = $row['Employer_ID_FK'];
                        
                        $qry = "SELECT * FROM bit WHERE Job_ID_FK = $jobid AND Employee_ID_FK = $user_id";
                        
                        $res = $conn->query($qry);

                        if($res->fetch_assoc()){
                            ?>
                                <button class="btn btn-outline-warning" disabled> Bited </button>
                                <a href="view_bit.php?bitedid=<?php echo  $row['Job_ID']; ?>" class="btn btn-outline-primary">View Bit</a>
                            <?php
                        }else{
                            ?>
                                <a href="bit_table.php?bitid=<?php echo  $row['Job_ID']; ?>" class="btn btn-outline-success">Bit</a>
                            <?php
                        }
