...
$query = "select * from employers where Approved_Status = 'true'";
$result = mysqli_query($conn,$query);

//Delete Query
if(isset($_GET['deleteid']))
{
    $Employer_ID  = $_GET['deleteid'];

    $sql = "DELETE FROM `employers` WHERE Employer_ID =$Employer_ID";
...
{
  $Employer_ID  = $_POST['approveid'];

  $sql = "Update `employers` set Approved_Status = 'false'  WHERE Employer_ID  = $Employer_ID";
  mysqli_query($conn, $sql);
  echo"<script>alert('Employer Permission Removed.');
  document.location.href = 'employer_pending.php'</script>";
}
        ...
        <div class="col-lg-12">
          <div id="form_ta" class="card">
            <div class="card-body">
              ...
              <div>
                <a href="employer_pending.php">
                  <button id="modal_btn" type="button" class="btn btn-outline-danger">Pending Employers</button>
                </a>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                        <form action="employer_table.php" method="post" >
                            <input name="approveid" type="hidden" value=<?php echo $row['Employer_ID'];  ?> />
                            <button type="submit" name="pendingButton" class="btn btn-success">Approved</button>
                        </form>
                    </td>

                    <td><a href="employer_update.php?updateid=<?php echo  $row['Employer_ID']; ?>" class="btn btn-primary">Update</a></td>
                    <td><a href="employer_table.php?deleteid=<?php echo  $row['Employer_ID']; ?>" class="btn btn-danger">Delete</a></td>
                    </tr>
                    <?php
                    }
                    ?>
